...
**What this repository contains:** only the code and data needed to reproduce the method's numeric results on [FinanceBench](https://github.com/patronus-ai/financebench). It does not include the full underlying corpus PDFs (159 MB -- fetch them with `download_pdfs.py`) or the derived retrieval index (>600 MB, regenerated by `prepare_embeddings_financebench.py`). It does include the ~189 gold-evidence pages (500 KB) natively bundled in FinanceBench's own public dataset (`all_150_questions.json`, CC BY-NC 4.0), via the `evidence_text_full_page` field.
